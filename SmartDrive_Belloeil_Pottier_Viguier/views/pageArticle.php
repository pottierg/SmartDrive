<!DOCTYPE html>
<html lang="fr">
 
 <head>
 
	<meta charset="utf-8" />
	<meta name="viewport" content="width=device-width, initial-scale=1.0" />
	<title>IHM Drive</title>
	<link rel="stylesheet" href="../css/foundation.css" />
	<script src="js/vendor/modernizr.js"></script>

</head>

<body>
<div class="small-block-grid-8" >
	
	<li style="background-color:#E6E6FA">
		<center>
			<div class="nomArticle" style="text-decoration: underline"><p><b>Nom Article</b></p></div>
			<div class="imageArticle"><p>Image</p></div>
			<div class="descriptionArticle"><p><i>Quid enim tam absurdum quam delectari multis inanimis rebus, 
			ut honore, ut gloria, ut aedificio, ut vestitu cultuque corporis, 
			animante virtute praedito, eo qui vel amare vel, ut ita dicam, redamare possit.</i>.</p></div>
			<div class="boutonAjouter"> <button type="button">Click Me!</button> </div>
		</center>
	</li>
	
</div>
</body>
</html>